...
export class SoundTouchAccessoryWrapper extends HomebridgeAccessoryWrapper<SoundTouchDevice> implements DeviceOnOffListener {
    ...
    constructor(context: HomebridgeContextProps, accessory: PlatformAccessory, device: SoundTouchDevice) {
        ...
        const _inf = this.initInformationService();
        if (!_inf) {
            throw new Error('Failure to get device information');
        }
        this.informationService = _inf;
    ...
    private initSourceServices(): Service[] {
        ...
        const sources = this.device.sources || [];
        for (let src of sources) {
            ...
            if (src.account === undefined) {
                console.error(`Account doesnt exist on src for ${src.account}`);
            }
    ...
    private initInformationService(): Service | undefined {
    ...
    private async _refreshDeviceServices(): Promise<void> {
        await new Promise(((resolve) => setTimeout(resolve, this.device.pollingInterval ?? 5000)));
        const selectedSource = await this.getSelectedSource();
        ...
        if (selectedSource !== undefined) {
            ...
            if (!deviceVolume) {
                console.error('Device volume not found')
            }
            if (this.volume) {
